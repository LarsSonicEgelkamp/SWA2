...
public class IbanUtil {
    ...
    /**
     * IBAN aus Ländercode, Kontonummer und BLZ erstellen und gleichzeitig validieren
     * 
     * @param countryCode
     * @param accountNumber
    ...
    public static String create(String countryCode, String accountNumber, String bankIdentification) {
        log.log(Level.INFO, String.format("Country: %1$s BankID: %2$8s Account: %3$10s", countryCode, bankIdentification, accountNumber));
        ...
        String iban = String.format("%1$s%2$02d%3$s%4$10s", countryCode, checkDigit, bankIdentification, accountNumber).replace(" ", "0");
        log.log(Level.INFO, String.format("Country: %1$s BankID: %2$8s Account: %3$10s => IBAN: %4$s", countryCode, bankIdentification, accountNumber, iban));
        return iban;
    }
